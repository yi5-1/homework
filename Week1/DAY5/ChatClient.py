...
def connect_to_server():
    ...
    if not username:
        ...
        return
    if not ip:
        messagebox.showwarning("提示", "請輸入伺服器 IP")
        return
    try:
...
tk.Label(login_frame, text="⚔ 連線至伺服器 ⚔", font=FONT_TITLE,
         fg=C_GOLD, bg=C_BG).pack(pady=(30, 15))

tk.Frame(login_frame, height=2, bg=C_GOLD).pack(fill=tk.X, padx=80)

tk.Label(login_frame, text="伺服器 IP", font=FONT, fg=C_CYAN, bg=C_BG).pack(pady=(10, 5))
ip_frame = tk.Frame(login_frame, bg=C_BG)
ip_frame.pack()
ip_entry = tk.Entry(ip_frame, width=18, font=FONT,
                    bg=C_BG2, fg=C_TEXT, insertbackground=C_GOLD,
                    relief=tk.FLAT, bd=6, highlightthickness=1,
                    highlightbackground=C_GOLD, highlightcolor=C_GOLD)
ip_entry.insert(0, "127.0.0.1")
ip_entry.pack(ipady=3)
tk.Label(ip_frame, text=":12345", font=FONT_BOLD, fg=C_GOLD, bg=C_BG).pack(side=tk.RIGHT, padx=(5, 0))

tk.Label(login_frame, text="使用者名稱", font=FONT, fg=C_CYAN, bg=C_BG).pack(pady=(10, 5))
username_entry = tk.Entry(login_frame, width=24, font=FONT,
                          bg=C_BG2, fg=C_TEXT, insertbackground=C_GOLD,
                          relief=tk.FLAT, bd=6, highlightthickness=1,
                          highlightbackground=C_GOLD, highlightcolor=C_GOLD)
username_entry.pack(pady=(0, 10), ipady=3)
...
connect_btn = tk.Button(login_frame, text="進 入 聊 天 室", command=connect_to_server,
                        ...
                        width=16, height=1)
connect_btn.pack(pady=(5, 0))

chat_frame = tk.Frame(root, bg=C_BG)

header = tk.Frame(chat_frame, bg=C_BG2, height=28)
header.pack(fill=tk.X, pady=(0, 3))
tk.Label(header, text="✦ 聊 天 室 ✦", font=FONT_BOLD,
         fg=C_GOLD, bg=C_BG2).pack(pady=4)

status_bar = tk.Frame(chat_frame, bg=C_BG, height=18)
status_bar.pack(side=tk.BOTTOM, fill=tk.X, padx=4, pady=(0, 3))
tk.Label(status_bar, text="連線中 ...", font=FONT_SM,
         fg=C_GREEN, bg=C_BG).pack(side=tk.LEFT)

input_frame = tk.Frame(chat_frame, bg=C_BG)
input_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=4, pady=(0, 3))

msg_entry = tk.Entry(input_frame, font=FONT, bg=C_BG2, fg=C_TEXT,
                     insertbackground=C_GOLD, relief=tk.FLAT, bd=6,
                     highlightthickness=1, highlightbackground=C_BG3,
                     highlightcolor=C_GOLD)
msg_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 4), ipady=3)
...
send_btn = tk.Button(input_frame, text="發送 ⏎", command=send_message,
                     ...
                     relief=tk.RAISED, bd=2, cursor="hand2", width=7)
...
chat_area = scrolledtext.ScrolledText(chat_frame, state=tk.DISABLED,
                                      ...
                                      highlightcolor=C_GOLD)
chat_area.pack(fill=tk.BOTH, expand=True, padx=4, pady=(0, 3))
