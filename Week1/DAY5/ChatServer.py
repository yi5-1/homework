...
def start_server():
    ...
    try:
        ...
        addr_label.config(text=f"{local_ip}:12345", fg=C_CYAN)
        log(f"[啟動] 伺服器位置: {local_ip}:12345")
...
def stop_server():
    ...
    addr_label.config(text="尚未啟動", fg=C_DIM)
    update_client_list()
    log("[停止] 伺服器已關閉。")
...
addr_label = tk.Label(top_frame, text="尚未啟動", font=FONT_SM,
                      fg=C_DIM, bg=C_BG2)
addr_label.pack(side=tk.LEFT, padx=(20, 0))
